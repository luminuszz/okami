...
import {
  SendEmailQueueConsumerDto,
  sendEmailQueueKey,
} from "@infra/mail/send-email-queue.consumer";
...
export class EmailService implements MailProvider {
  ...
  async publishInEmailQueue(payload: SendEmailQueueConsumerDto): Promise<void> {
    await this.queue.publish<SendEmailQueueConsumerDto>(
      sendEmailQueueKey,
      payload,
    );
  }
  ...
  async sendResetPasswordEmail({
    email,
    resetPasswordCode,
  }: SendResetPasswordEmailDto): Promise<void> {
  ...
  async sendConfirmEmail({
    confirmEmailCode,
    email,
  }: SendConfirmEmailDto): Promise<void> {
